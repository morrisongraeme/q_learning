...
class Agent:
    ...
    def __init__(self, n_states, n_actions, discount=0.99, min_explore=0.01, max_explore=1,
                 ...
                 action_remap_gain=1, update_target_interval=1000):
...
class Brain:
    ...
    def _create_sess(self):
        ...
        self.b1_t = bias_variable([self.n_hidden_neurons])
        self.w2_t = tf.get_variable("w2_t", shape=[self.n_hidden_neurons, self.n_actions])
        self.b2_t = bias_variable([self.n_actions])
        ...
        q_out_t = tf.matmul(h1_t, self.w2_t) + self.b2_t
        self.prediction = q_out
        self.prediction_t = q_out_t
